...
BEGIN
    ...
    IF v_bonus_amount > 0 THEN
        RAISE NOTICE 'Program 1: Moderator % received a bonus of $% for the month %/%!', v_mod_name, v_bonus_amount, v_month, v_year;
    ELSE
        RAISE NOTICE 'Program 1: Moderator % has no bonus in the month %/% (did not close any investigations resulting in bans this month).', v_mod_name, v_month, v_year;
    END IF;
END;
$$;
...
DECLARE
    ...
    v_extra_days INT := 14;
BEGIN
    RAISE NOTICE 'Program 2: Attempting to extend Ban ID % by % days...', v_ban_id_to_extend, v_extra_days;
    
    -- קריאה לפרוצדורה (שתזרוק אקספשן כי ה-ID לא קיים)
    CALL extend_ban_for_repeat_offenders(v_ban_id_to_extend, v_extra_days);
    
    RAISE NOTICE 'Program 2: Operation completed successfully with no errors.';

EXCEPTION
    WHEN OTHERS THEN
        -- תפיסת האקספשן שנזרק מהפרוצדורה (או כל שגיאה אחרת)
        RAISE NOTICE 'Program 2 caught an error during execution: %', SQLERRM;
END;
$$;
